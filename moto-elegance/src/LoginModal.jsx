...
export default function LoginModal({ onNavigateToRegister, onClose }) {
  ...
  const { login, validateAdminCredentials } = useAuth();
  ...
  const handleContinue = async (e) => {
    ...
    if (email && !isPasswordView) {
      // Show password field after email is entered
    ...
    try {
      ...
      const isAdmin = validateAdminCredentials(email, password);

      if (isAdmin) {
        // If admin credentials, don't make API call, just log in as admin
        const adminToken = `admin-${Date.now()}`; // Generate a unique token for this session
        ...
        // First store admin token in localStorage
        localStorage.setItem("token", adminToken);
        localStorage.setItem("user", JSON.stringify(adminUser));

        // THEN use the login function from context to update auth state
        // This ensures that if login() uses the values, they're already set
        login(adminUser);
      ...
      // If not admin, proceed with normal API login
      console.log("Attempting to log in via API:", email);
  ...
  const handleSocialLogin = (provider) => {
    // In a real app, you would implement OAuth login
    setError("Social login is not implemented yet");
    // Uncomment below for demo purposes
    // login({ email: `user@${provider}.com`, firstName: "Demo" });
    // onClose();
  };
              ...
              <form onSubmit={handleContinue} className="mt-6">
                {!isPasswordView ? (
                  <>
                    ...
                    <button
                      type="submit"
                      className="mt-4 w-full rounded-md bg-black px-3 py-3 text-sm font-medium text-white shadow-sm hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                    >
                      Continue
                    </button>
                  ...
                  <>
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className={`w-full rounded-md px-3 py-3 text-sm font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${
